...
const callAI = async (messages, system) => {
  const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: MODEL, max_tokens: 150, messages: [{ role: "system", content: system }, ...messages] }),
  });
...
.nav{display:flex;gap:6px;background:var(--card);border:1px solid var(--border);border-radius:16px;padding:6px}
.nav-btn{flex:1;padding:9px 4px;border:none;border-radius:12px;background:transparent;color:var(--muted);font-family:'Nunito',sans-serif;font-weight:800;font-size:12px;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:4px;white-space:nowrap}
...
.game-start-btn:hover{transform:translateY(-2px);box-shadow:0 6px 28px #f5c84260}
...
function WaterGlass({ filled, total }) {
  ...
  return (
    <svg viewBox="0 0 100 140" className="water-glass-svg">
      <defs>
        <clipPath id="glassClip"><path d="M15 10 L10 130 Q10 135 15 135 L85 135 Q90 135 90 130 L85 10 Z" /></clipPath>
...
function WaterPage() {
  const [glasses, setGlasses] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem("gl_water") || "{}"); return s.date === TODAY_KEY ? (s.glasses || 0) : 0; } catch { return 0; }
  });
  ...
  useEffect(() => {
    ...
    if (glasses >= WATER_GOAL && !goalShown) { setWaterConfetti(true); setGoalShown(true); setTimeout(() => setWaterConfetti(false), 4000); }
  }, [glasses, goalShown]);
  ...
  const addGlass = () => {
    ...
    setGlasses(g => g + 1);
    const id = Date.now();
    setSplashes(prev => [...prev, { id, x: 30 + Math.random() * 40 }]);
  ...
  return (
    <div className="water-page">
      ...
      <div className="water-hero">
        ...
        <div className={`water-tagline ${currentMsg.dry ? "dry" : "good"}`}>{currentMsg.msg}</div>
      </div>
      <div className="card">
        ...
        <div className="glasses-grid">
          {Array.from({ length: WATER_GOAL }, (_, i) => (
            <button key={i} className={`glass-btn ${i < glasses ? "filled" : ""}`}
              onClick={() => i < glasses ? setGlasses(i) : i === glasses && addGlass()}>
              {i < glasses ? "💧" : "🥛"}
            </button>
          ))}
        </div>
...
// ── BRICK BREAKER ──────────────────────────────────────────────────────────
const CW = 360, CH = 480;
const PAD_W = 70, PAD_H = 10, PAD_Y = CH - 30;
const BALL_R = 8;
...
function makeBricks() {
  ...
  for (let r = 0; r < BRICK_ROWS; r++)
    for (let c = 0; c < BRICK_COLS; c++)
      bricks.push({ r, c, alive: true, color: BRICK_COLORS[r % BRICK_COLORS.length] });
  return bricks;
}
...
function GamePage() {
  ...
  const stateRef = useRef({
    status: "idle", // idle | playing | paused | lost | won
    padX: CW / 2 - PAD_W / 2,
    ball: { x: CW / 2, y: PAD_Y - BALL_R - 1, vx: 3, vy: -4 },
    bricks: makeBricks(),
    score: 0,
    lives: 3,
    highScore: parseInt(localStorage.getItem("gl_hs") || "0"),
    totalBricks: BRICK_COLS * BRICK_ROWS,
  });
  ...
  const draw = useCallback(() => {
    ...
    s.bricks.forEach(b => {
      if (!b.alive) return;
      const x = BRICK_OFFSET_X + b.c * (BRICK_W + BRICK_PAD);
      const y = BRICK_OFFSET_Y + b.r * (BRICK_H + BRICK_PAD);
    ...
    ctx.shadowBlur = 12;
    roundRect(ctx, s.padX, PAD_Y, PAD_W, PAD_H, 5);
    ctx.fill();
    ctx.shadowBlur = 0;
  ...
  const update = useCallback(() => {
    ...
    // Paddle bounce
    if (s.ball.y + BALL_R >= PAD_Y && s.ball.y - BALL_R <= PAD_Y + PAD_H &&
        s.ball.x >= s.padX && s.ball.x <= s.padX + PAD_W) {
      const hit = (s.ball.x - s.padX) / PAD_W;
      const angle = (hit - 0.5) * 2.4;
    ...
    if (s.ball.y > CH + BALL_R) {
      ...
      if (s.lives <= 0) {
        s.status = "lost";
        if (s.score > s.highScore) { s.highScore = s.score; localStorage.setItem("gl_hs", s.score); }
        setDisplay(d => ({ ...d, status: "lost", lives: 0, score: s.score, highScore: s.highScore }));
        return;
      }
      s.ball = { x: CW / 2, y: PAD_Y - BALL_R - 1, vx: 3 * (Math.random() > 0.5 ? 1 : -1), vy: -4 };
    ...
    s.bricks.forEach(b => {
      if (!b.alive) return;
      const bx = BRICK_OFFSET_X + b.c * (BRICK_W + BRICK_PAD);
      const by = BRICK_OFFSET_Y + b.r * (BRICK_H + BRICK_PAD);
      if (s.ball.x + BALL_R > bx && s.ball.x - BALL_R < bx + BRICK_W &&
    ...
    if (s.bricks.every(b => !b.alive)) {
      s.status = "won";
      if (s.score > s.highScore) { s.highScore = s.score; localStorage.setItem("gl_hs", s.score); }
      setDisplay(d => ({ ...d, status: "won", score: s.score, highScore: s.highScore }));
    }
  }, []);
  ...
  const startGame = useCallback(() => {
    ...
    s.status = "playing";
    s.padX = CW / 2 - PAD_W / 2;
    s.ball = { x: CW / 2, y: PAD_Y - BALL_R - 1, vx: 3, vy: -4 };
    s.bricks = makeBricks();
    s.score = 0;
    s.lives = 3;
    setDisplay({ status: "playing", score: 0, lives: 3, highScore: s.highScore });
  ...
  const handlePointer = useCallback((e) => {
    ...
    stateRef.current.padX = Math.max(0, Math.min(CW - PAD_W, x - PAD_W / 2));
  }, []);

  const movePad = (dir) => {
    const s = stateRef.current;
    if (dir === "left") s.padX = Math.max(0, s.padX - 30);
    else s.padX = Math.min(CW - PAD_W, s.padX + 30);
  };
    ...
    <div className="game-page">
      <div className="game-header" style={{width:"100%"}}>
        ...
        <div className="game-scores">
          ...
          <div className="score-chip">Can
            <div className="lives">
              {Array.from({length:3},(_,i)=><span key={i} className={`life ${i >= display.lives ? "lost":""}`}>❤️</span>)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="game-canvas-wrap">
        <canvas ref={canvasRef} width={CW} height={CH}
          onMouseMove={handlePointer} onTouchMove={handlePointer} style={{touchAction:"none"}} />
        {(display.status === "idle" || display.status === "lost" || display.status === "won") && (
          <div className="game-overlay">
            <div className="overlay-title">
              {display.status === "idle" && "🧱 Brick"}
              {display.status === "lost" && "💀 Oyun Bitti"}
              {display.status === "won" && "🏆 Kazandın!"}
            </div>
            {display.status !== "idle" && <div className="overlay-score">Skor: {display.score}</div>}
            <div className="overlay-sub">
              {display.status === "idle" && "Klavyeyle veya aşağıdaki\nbutonlarla oyna!"}
              {display.status === "lost" && "Daha iyi şans bir dahaki\nsefere... 😅"}
              {display.status === "won" && "Tüm tuğlaları kırdın!\nEfsane misin sen? 🔥"}
            </div>
            <button className="game-start-btn" onClick={startGame}>
              {display.status === "idle" ? "🎮 Oyunu Başlat" : "🔄 Tekrar Oyna"}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="game-controls">
        <button className="ctrl-btn" onPointerDown={() => keysRef.current["ArrowLeft"]=true} onPointerUp={() => keysRef.current["ArrowLeft"]=false} onPointerLeave={() => keysRef.current["ArrowLeft"]=false} onClick={() => movePad("left")}>◀</button>
        <button className="ctrl-btn" onPointerDown={() => keysRef.current["ArrowRight"]=true} onPointerUp={() => keysRef.current["ArrowRight"]=false} onPointerLeave={() => keysRef.current["ArrowRight"]=false} onClick={() => movePad("right")}>▶</button>
      </div>
      <div style={{fontSize:12,color:"var(--muted)",textAlign:"center",lineHeight:1.6}}>
        🖥️ Klavye: ← → tuşları &nbsp;|&nbsp; 📱 Mobil: Dokunarak veya ◀▶ butonlarıyla
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [permDone, setPermDone] = useState(() => {
    try { const s = JSON.parse(localStorage.getItem("gl_perm_done") || "{}"); return s.date === TODAY_KEY ? (s.done || {}) : {}; }
    catch { return {}; }
  });
  ...
  useEffect(() => {
    ...
    const allD = fd.length > 0 && fd.every(t => t.done);
    const allP = permanentTasks.length === 0 || permanentTasks.every(t => permDone[t.id]);
    if (allD && allP && !confettiShown && (fd.length > 0 || permanentTasks.length > 0)) {
  ...
  const selectMood = async (m) => {
    ...
    try {
      const text = await callAI([{ role:"user", content:`Kullanıcı bugün "${m.label}" hissediyor. Kısa, samimi karşıla. Türkçe, 2-3 cümle.` }], "Sen günlük bir yaşam koçusun. Sıcak ve pozitif.");
      setAiMsg(text);
  ...
  const sendChat = async () => {
    ...
    setChatMessages(p => [...p, { role:"user", content:userMsg }]); setChatLoading(true);
    try {
      const text = await callAI([...chatMessages, { role:"user", content:userMsg }],
        `Günlük yaşam koçusun. Ruh hali: ${mood?.label || "?"}\nTürkçe, kısa, max 3 cümle.`);
      setChatMessages(p => [...p, { role:"assistant", content:text }]);
    } catch { setChatMessages(p => [...p, { role:"assistant", content:"Bir hata oluştu!" }]); }
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <div className="nav">
          <button className={`nav-btn ${page==="home"?"active":""}`} onClick={() => setPage("home")}>✅ Görevler</button>
        ...
        {page === "home" && (
          <>
            ...
            <div className="card">
              <div className="card-label"><span>Daimi Görevler</span><span style={{fontSize:10,color:"#6b6b9a"}}>Her gün tekrarlanır</span></div>
              ...
              <div className="add-row">
                <input className="add-input" value={newPermInput} onChange={e => setNewPermInput(e.target.value)} onKeyDown={e => e.key==="Enter"&&addPermanent()} placeholder="Yeni daimi görev ekle..." />
            ...
            <div className="card">
              ...
              <div className="tasks-list">
                {dailyTasks.map((task, i) => (
                  <div key={task.id} className={`task-item ${task.done?"done":""}`}>
                    ...
                    <input className="task-input" value={task.text} onChange={e => updateDaily(task.id, e.target.value)}
                      placeholder={["En önemli görevin...","İkinci görevin...","Üçüncü görevin..."][i]||"Görev ekle..."} disabled={task.done} />
                    {task.text.trim() && <>
              ...
              <div className="add-row" style={{marginTop:10}}>
                <input className="add-input" value={newDailyInput} onChange={e => setNewDailyInput(e.target.value)} onKeyDown={e => e.key==="Enter"&&addDailyTask()} placeholder="Ekstra görev ekle..." />
            ...
            {mood && (
              <div className="card">
                ...
                <div className="chat-row">
                  <textarea className="chat-field" rows={2} value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),sendChat())} placeholder="Bir şey sor veya anlat..." />
